...
if __name__ == '__main__':
    ...
    parser.add_argument("--embedding_dim", type=int, dest="embedding_dim", help="Embedding dimension", default=8)
    parser.add_argument("--hidden_dim", type=int, dest="hidden_dim", help="Hidden dimension", default=8)
    parser.add_argument("--verbose", dest="verbose", help="Verbose", action="store_true", default=True)
    ...
    if MODEL == "RL":
        ...
        if STATE_ABSTRACTION == "random":
            ...
            if STATE_ABSTRACTION == "sequence":
                ...
                oracle_kwargs["abstract_state_fn"] = parent_state_ngram_fn(4, MAX_DEPTH)

            elif STATE_ABSTRACTION == "left_right_tree":
                oracle_kwargs["abstract_state_fn"] = left_right_parent_state_ngram_fn(4, MAX_DEPTH)
        ...
        fuzz_kwargs["oracle"] = GFNOracle_flow_matching(128, 128, DOMAINS, epsilon=EPSILON)

    elif MODEL == "TB":
        fuzz_kwargs["oracle"] = GFNOracle_trajectory_balance(128, 128, DOMAINS, epsilon=EPSILON)

    elif MODEL == "DB":
        fuzz_kwargs["oracle"] = GFNOracle_detailed_balance(128, 128, DOMAINS, epsilon=EPSILON)

    elif MODEL == "LS":
        fuzz_kwargs["oracle"] = GFNOracle_local_search(128, 128, DOMAINS, epsilon=EPSILON)
