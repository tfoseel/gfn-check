...
def initialize_model(vocab_size, embedding_dim=8, num_actions=16):
    ...
    lstm_layer = nn.LSTM(input_size=embedding_dim, hidden_size=num_actions, batch_first=True)
...
class GFNLearner:
    ...
    def policy(self, domain, state):
        ...
        if np.random.binomial(1, self.epsilon):
            ...
            self.action_values = process_sequence(state, self.embedding_layer_pf, self.lstm_layer_pf)
